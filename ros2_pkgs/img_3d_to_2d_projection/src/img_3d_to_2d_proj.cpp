...
void DepthToImageProjection::OnCameraMsg(const sensor_msgs::msg::Image::SharedPtr _msg){
	...
	std::vector<float> objects = this->objects_dists;
	std::vector<float> xz_angle = this->objects_xz_angle;
	std::vector<float> yz_angle = this->objects_yz_angle;
	int closest_index = this->closest_idx;

	for (size_t i = 0; i < objects.size(); i++)
	{
		// horizontal pixel
		float x_depth = sin(xz_angle.at(i)) * objects.at(i);
		float y_depth = cos(xz_angle.at(i)) * objects.at(i);
		...
		// vertical pixel
		x_depth = sin(yz_angle.at(i)) * objects.at(i); 
		y_depth = cos(yz_angle.at(i)) * objects.at(i);
	...
	if (objects.size() > 0)
	{
		RCLCPP_INFO(this->get_logger(),  "\n Shortest dist pixel: \n x_px: %f, \n y_px: %f", x_px_vec.at(closest_index), y_px_vec.at(closest_index));
	}
	else {
					...
					if (x_px_vec.at(i) > 0 && x_px_vec.at(i) < _msg->width && y_px_vec.at(i) > 0 && y_px_vec.at(i) < _msg->height)
					{
						...
						{		
							...
							if( draw_idx < num_img_vals && draw_idx > -1){
								_msg->data.at(draw_idx) = 0; // draw black pixel
							}
						} 
						else 
						{
